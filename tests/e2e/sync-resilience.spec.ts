...
test('a locally applied timeline repair remains uploadable after reload', async ({ page }) => {
  await waitForApp(page);

  const repairId = await page.evaluate(async () => {
    const backups = await import('/src/services/workspaceBackup.ts');
    const repairs = await import('/src/services/workspaceTimelineRepair.ts');
    const backup = backups.createWorkspaceBackup();
    backup.timeline.tasks = [{
      id: 'repair-retry-task',
      name: 'repair retry task',
      start: '2026-09-03',
      end: '2026-09-03',
      blocks: [{ type: 'text', id: '', content: 'must survive reload' }],
    }] as unknown as typeof backup.timeline.tasks;
    backup.timeline.groups = [];
    const prepared = await repairs.prepareTimelineBlocksRepair({
      workspaceId: 'workspace-repair-retry',
      local: backup,
      remoteRoot: { tasks: backup.timeline.tasks, groups: backup.timeline.groups },
    });
    await repairs.executePreparedTimelineBlocksRepair(backup, prepared);
    const uploads = await repairs.listPendingTimelineBlocksRepairUploads();
    if (!uploads.some((upload) => upload.repairId === prepared.plan.repairId)) {
      throw new Error('repair upload was not persisted');
    }
    return prepared.plan.repairId;
  });

  await page.reload();
  await expect(page.locator('.tl-dock')).toBeVisible();
  await expect.poll(() => page.evaluate(async (expectedRepairId) => {
    const repairs = await import('/src/services/workspaceTimelineRepair.ts');
    const uploads = await repairs.listPendingTimelineBlocksRepairUploads();
    return uploads.some((upload) => upload.repairId === expectedRepairId);
  }, repairId)).toBe(true);
});
