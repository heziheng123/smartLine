...
## Data-safety review closeout

The final review found and closed two additional loss windows:

- A repaired local timeline could be committed while its cloud upload failed.
  The repaired payload is now stored in `workspace_repairs` in the same
  IndexedDB transaction as the repaired timeline, history entry, and receipt.
  A later flush resumes that exact payload before attempting a new repair.
- Queue acknowledgement previously proved only the materialized entity view.
  It now requires both the raw schema 8 top-level field and the materialized
  entity projection to match the submitted value before dequeue.

If the remote timeline changed after the repair snapshot, the durable upload is
retained and the remote value is not overwritten. If a newer local queue
revision exists after repair, only the older exact `tasks`/`groups` payload may
be acknowledged; the newer queue remains pending.

## Sequential rollback gates

1. **Diagnosis and backup** — read-only integrity reporting, mutation-origin
   enforcement, complete hashed snapshot/manifest, and storage transaction
   primitives. No cloud writer changes.
2. **ID repair** — persistent source-hashed `timeline.blocks` RepairPlan,
   atomic local apply/history/receipt/upload payload, and resumable upload.
3. **Merge and dequeue** — remote-current leaf conflicts, durable alternates,
   unrelated-field progress, dual-projection readback, and exact queue
   acknowledgement.
4. **UI and E2E baseline** — non-blocking recovery UI plus root-cause repairs
   for the existing desktop and small-screen E2E failures. This gate does not
   skip tests or raise the global Playwright timeouts.

Schema 9 and dependency upgrades remain outside all four gates.
