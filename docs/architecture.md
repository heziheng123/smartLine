...
人生地图已从旧 Timeline 类型分离。面向用户提供项目、长期系统、关键日期、时期重点和文字便签的统一添加入口；项目子阶段是项目内部能力，人生时期、二级分类、复盘和日期调整收纳在规划概览。目标实体已从所有 UI 投影排除；底层 `kind: 'goal'` 只为历史同步和备份兼容保留，不影响范围、搜索、统计、空状态或新复盘。
